...
client.on('messageCreate', async message => {
    ...
    if (message.author.id === ownerId) {
        ...
        if (command === 'autodel') {
            await toggleAutoDelete(message, args);
        }

        if (command === 'add') {
            await addAccountManually(message, args);
        }
...
async function handleFollowCommand(message, args) {
    ...
    // Check if we have enough accounts - if not, still proceed but show processing message
    if (accounts.length < followers) {
        // Show processing message even with insufficient stock
        const embed = new MessageEmbed()
            .setTitle('🚀 Processing Request!')
            .setDescription(`Attempting to send ${followers} followers to **${username}**\n\n**Note:** We have ${accounts.length} accounts available. Will send what we can!`)
            .setColor('#ffff00')
            .setTimestamp();
    ...
    try {
        ...
        if (result.success) {
            ...
            const embed = new MessageEmbed()
                ...
                .setColor('#00ff00')
                .setTimestamp()
                .setFooter('discord.gg/Rofollow', 'https://i.imgur.com/your-banner-url.png');
            ...
            const embed = new MessageEmbed()
                .setTitle('❌ Failed to Send Followers')
                .setDescription(`Failed to send followers to **${username}**: ${result.error}`)
                .setColor('#ff0000')
                .setTimestamp();
        ...
        const embed = new MessageEmbed()
            ...
            .setDescription('An error occurred while sending followers. Please try again later.')
            .setColor('#ff9900')
            .setTimestamp();
...
async function showProxies(message) {
    const embed = new MessageEmbed()
        .setTitle('Proxy Status')
        .setDescription(`**Total Proxies Available:** ${proxies.length}`)
        .setColor('#0099ff')
        .setTimestamp();
...
async function toggleAutoDelete(message, args) {
    const action = args[0]?.toLowerCase();
    
    if (action === 'on') {
        if (autoDeleteEnabled) {
            return message.channel.send('Auto-delete is already enabled!');
        }
        ...
        autoDeleteInterval = setInterval(async () => {
            try {
                ...
                const messagesToDelete = messages.filter(msg => 
                    ...
                    (Date.now() - msg.createdTimestamp) > 20000 // 20 seconds
                );
                ...
                logger.error('Error during auto-delete:', error);
            }
        }, 20000); // Check every 20 seconds
        
        const embed = new MessageEmbed()
            .setTitle('Auto-Delete Enabled')
            .setDescription('Auto-delete is now **ON**. Messages will be deleted every 20 seconds (pinned messages will be preserved).')
            .setColor('#00ff00')
            .setTimestamp();
        ...
        if (!autoDeleteEnabled) {
            return message.channel.send('Auto-delete is already disabled!');
        }
        ...
        const embed = new MessageEmbed()
            ...
            .setDescription('Auto-delete is now **OFF**. Messages will no longer be automatically deleted.')
            .setColor('#ff0000')
            .setTimestamp();
        ...
        const embed = new MessageEmbed()
            .setTitle('Auto-Delete Usage')
            .setDescription('Usage: `$autodel on` to enable or `$autodel off` to disable\n\nWhen enabled, messages older than 20 seconds will be automatically deleted (pinned messages are preserved).')
            .setColor('#ffff00')
            .setTimestamp();
...
async function addAccountManually(message, args) {
    if (!args[0]) {
        return message.reply('Please provide a cookie. Usage: $add "cookie_value"');
    }
    ...
    const embed = new MessageEmbed()
        .setTitle('Account Added Manually')
        .setDescription(`Successfully added manual account to pre-stock.\n**Cookie:** ${cookie.substring(0, 50)}...`)
        .setColor('#00ff00')
        .setTimestamp();
    ...
    const logChannel = client.channels.cache.get(logChannelId);
    if (logChannel) {
        logChannel.send(`Owner manually added account. Total pre-stock: ${prestockAccounts.length}`);
    }
}
